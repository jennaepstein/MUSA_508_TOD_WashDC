...
## Visualizing Percent of People Who Commute by Public Transportation
```{r task2_PercentPubCommute, include=TRUE, message=FALSE, warning=FALSE}
mapPubCommute <-
  ggplot(allTracts.group)+
  geom_sf(data = st_union(tracts2009))+
...
```{r comparison_groupedbars, include=TRUE, message=FALSE, warning=FALSE}
allTracts.Summary %>%
  gather(Variable, Value, -year, -TOD) %>%
...
```{r graduated_symbol_map, include=TRUE, message=FALSE, warning=FALSE}
# map
...
```{r crime_data_wrangling, include=TRUE, message=FALSE, warning=FALSE}
...
```{r multiple_ring_buffer, include=TRUE, message=FALSE, warning=FALSE}
# MULTIPLE RING BUFFER -----------------
